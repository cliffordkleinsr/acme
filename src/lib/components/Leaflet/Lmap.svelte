<script lang="ts">
	...
	onMount(async () => {
		...
		function onEachFeature(
			...
			layer: {
				on: (arg0: {
					mouseover: (e: { target: any }) => void;
					mouseout: (e: { target: any }) => void;
					click: (e: { target: { getBounds: () => LatLngBoundsExpression } }) => void;
				}) => void;
			}
		) {
			layer.on({
...
<div class="z-10 hidden h-full sm:block" bind:this={mapEl}></div>
