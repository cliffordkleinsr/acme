...
<div class="grid gap-3 sm:grid-cols-1 md:grid-cols-2 2xl:grid-cols-3">
	...
	{#if !checked}
		...
		<div
			transition:fly={{ delay: 350, duration: 300, x: 0, y: 300, opacity: 0.5, easing: sineInOut }}
		>
			{#if gen_act}
				<Select.Root
	...
	<AlertDialog.Root bind:open={dialog}>
		<AlertDialog.Trigger asChild let:builder>
			<Button builders={[builder]} class="w-full"><Flame class="mr-2 size-4" />{default_txt}</Button
			>
		</AlertDialog.Trigger>
		<AlertDialog.Content>
			<AlertDialog.Header>
