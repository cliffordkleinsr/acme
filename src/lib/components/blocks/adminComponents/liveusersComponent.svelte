...
{#if users.length === 0}
	<p class="text-sm italic">No users curently active</p>
	<p class="text-sm italic">
		This pane will fill up with information showing all users currently active
	</p>
{:else}
	{#each users as usr}
		<div class="flex items-center gap-4">
			{#if usr?.gender === 'male'}
				...
				<span class="relative bottom-3 right-14 z-10 flex h-3 w-3">
					<span
			...
			<div class="ml-auto text-sm font-medium">Active {timeAgo(usr.at)}</div>
		</div>
		<Separator />
	{/each}
{/if}
