...
{/if}
<div class="m-4 mt-2 flex flex-col gap-10">
	<div class="mb-2 ml-5">
		<Breadcrumb.Root>
			<Breadcrumb.List>
				<Breadcrumb.Item>
	...
	<div class="grid gap-4 md:grid-cols-2 md:gap-8 lg:grid-cols-3">
		<Card.Root class="max-w-xl space-y-3">
			<Card.Header class="mb-2 ">
				...
				<Card.Title class="grid grid-cols-3 gap-10 text-4xl">
					<p>{total_points}</p>
					<p>{total_paid}</p>
					<p>{total_payable}</p>
				</Card.Title>
		...
		<Card.Root class="lg:max-w-sm">
			<Card.Header class="mb-2">
				...
				<Card.Content>
					<Button class="flex w-full gap-2" variant="secondary" href="/agent-dash/surveys/take">
						Take a survey
						<ArrowUpRight class="size-4" />
					</Button>
				</Card.Content>
	...
	{#if count.length > 0}
		<Chart class="ml-5 mt-5 max-w-2xl" {options} />
	{:else}
