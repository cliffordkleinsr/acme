...
{#if answers.length > 0}
	<div class="m-1 flex gap-1">
		...
		<Sheet.Root>
			<Sheet.Trigger
				class={cn(buttonVariants({ variant: 'secondary', size: 'default' }), 'flex gap-1')}
				><ListFilter class="size-4" /> Filters</Sheet.Trigger
			>
			<Sheet.Content>
				<Sheet.Header>
					...
					<Sheet.Description>
						<div class="grid gap-5">
							<div class="grid gap-2">
								<h1 class="text-md text-balance font-semibold">Gender</h1>
							...
							<div class="grid gap-2">
								<h1 class="text-md text-balance font-semibold">Timeframe</h1>
								<Picker />
							</div>
						</div>
					</Sheet.Description>
		...
		{:else if male || female}
			<Badge variant="secondary" class="text-muted-foreground"
				><X class="size-4" /> {male ? 'Male' : 'female'}</Badge
			>
		{/if}
	</div>
	<div class="m-1 grid w-full gap-2" id="my-div">
		<div class="grid gap-2 sm:grid-cols-2 lg:grid-cols-4">
			...
			<Card.Root class="col-span-2">
				<Card.Header>
					<Card.Title class="text-end text-5xl">Summary Statistics</Card.Title>
									...
									<Dialog.Root>
										<Dialog.Trigger asChild let:builder>
											<Button variant="ghost" size="icon" builders={[builder]}
												><PieChart class="size-4 text-muted-foreground" /></Button
											>
										</Dialog.Trigger>
										<Dialog.Content>
											<Dialog.Header>
						...
						<Card.Root>
							...
							<Card.Content>
								<Table.Root>
									...
									<Table.Body>
										{#each question.results as res, ix}
											<Table.Row class="mx-auto space-y-2 lg:mx-px">
												<Table.Cell class="font-normal">A{ix + 1}</Table.Cell>
												<Table.Cell class=" line-clamp-3 font-normal">{res.answer}</Table.Cell>
												<Table.Cell class="font-normal">
													<Progress value={res.percentage} class="h-2 w-5 lg:w-[80%]" />
												</Table.Cell>
												<Table.Cell class="text-right font-normal"
													>{Math.round(res.percentage)}%</Table.Cell
												>
