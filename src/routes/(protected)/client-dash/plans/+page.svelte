<script lang="ts">
    ...
    let date: Date | null = client_pack.at
	const price = client_pack.plan!
	const vat = parseInt(price!) * 0.16

	
    
</script>

{#if client_pack.packageid !== null}
<div id='section-to-print' class="w-full">
<Card.Root class="overflow-hidden m-4 w-1/2">
	<Card.Header class="flex flex-row items-start bg-muted/50">
		<div class="grid gap-0.5">
			<Card.Title class="group flex items-center gap-2 text-lg">
				Package {client_pack.packageid} 
				<Button
					size="icon"
					variant="outline"
		...
		<div class="ml-auto flex items-center gap-1">
			...
			<DropdownMenu.Root>
                ...
                <Button variant='ghost' class='w-full' on:click={() => window.print()}>Export</Button>
