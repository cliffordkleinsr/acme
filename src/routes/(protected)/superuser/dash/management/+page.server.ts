...
export const actions: Actions = {
	deleteSurvey: async ({ request }) => {
		...
		// console.log(data)
		try {
			await db.delete(progressTable).where(eq(progressTable.surveyid, id));
			await db.delete(agentSurveysTable).where(eq(agentSurveysTable.surveyid, id));
			await db.delete(AnswersTable).where(eq(AnswersTable.surveid, id));
			await db.delete(QuestionOptions).where(sql`
